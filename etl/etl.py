...
def normalize_actor_obj(a: dict) -> tuple[int, str, str | None, str | None]:
    """
    Expected actor object shapes (partner/CMS):
      {"actor_id": 101, "full_name": "Leonardo DiCaprio", "role_name": "Cobb", "birth_date": "1974-11-11"}
    role_name and birth_date are optional.
    """
    actor_id = int(a["actor_id"])
    full_name = clean(a.get("full_name")) or ""
    birth_date = a.get("birth_date")  # "YYYY-MM-DD" or None
...
actors_rows: dict[int, tuple[int, str, str | None]] = {}  # actor_id -> (actor_id, full_name, birth_date)
links_rows: list[tuple[int, int, str | None]] = []         # (movie_id, actor_id, role_name)
...
for p in partner:
    ...
    partner_actors_raw = p.get("actors", []) or []  # list of actor objects
